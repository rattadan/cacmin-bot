...
export function registerFineConfigCommands(bot: Telegraf<Context>): void {
  ...
  bot.command('setfine', ownerOnly, async (ctx) => {
    ...
    const validTypes = ['sticker', 'url', 'regex', 'blacklist', 'jail_per_minute', 'jail_minimum', 'auto_jail'];
    if (!validTypes.includes(fineType)) {
      return ctx.reply(` Invalid fine type. Valid types: ${validTypes.join(', ')}`);
    }
  ...
  bot.command('listfines', ownerOnly, async (ctx) => {
    ...
    const allTypes = ['sticker', 'url', 'regex', 'blacklist', 'jail_per_minute', 'jail_minimum', 'auto_jail'];
  ...
  bot.command('junoprice', ownerOnly, async (ctx) => {
    ...
    const message = '*JUNO Price Information*\n\n' +
  ...
  bot.command('customjail', ownerOnly, async (ctx) => {
    ...
    if (isNaN(junoAmount) || junoAmount < 0) {
      return ctx.reply(' Invalid fine amount. Please enter a non-negative number.');
    }
    ...
    logger.info('User custom jailed', { ownerId, userId, minutes, junoAmount, reason });
  });
  ...
  bot.command('initfines', ownerOnly, async (ctx) => {
    ...
    const defaults: Array<{ type: string; amount: number; desc: string }> = [
      { type: 'sticker', amount: 0.10, desc: 'Restricted sticker violation' },
      { type: 'url', amount: 0.20, desc: 'URL posting violation' },
      { type: 'regex', amount: 0.15, desc: 'Regex pattern violation' },
      { type: 'blacklist', amount: 0.50, desc: 'Blacklist violation' },
      { type: 'jail_per_minute', amount: 0.01, desc: 'Per-minute jail rate' },
      { type: 'jail_minimum', amount: 0.10, desc: 'Minimum jail fine' },
      { type: 'auto_jail', amount: 1.00, desc: 'Auto-jail fine' }
    ];

    for (const def of defaults) {
      // Only set if not already configured
      const existing = PriceService.getAllFineConfigs().find(c => c.fine_type === def.type);
      if (!existing) {
