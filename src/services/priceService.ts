...
export class PriceService {
  ...
  static async fetchCurrentPrice(): Promise<number | null> {
    ...
    try {
      const response = await fetch(
        `https://api.coingecko.com/api/v3/simple/price?ids=${this.COINGECKO_ID}&vs_currencies=usd`
      );

      if (!response.ok) {
        StructuredLogger.logError(new Error(`CoinGecko API error: ${response.status}`), {
          operation: 'fetch_price'
        });
        return this.cachedPrice; // Return stale cache on error
      }

      const data = await response.json() as Record<string, { usd?: number }>;
      const price = data[this.COINGECKO_ID]?.usd;
  ...
  static async getRollingAveragePrice(): Promise<number> {
    const cutoff = Math.floor(Date.now() / 1000) - (this.ROLLING_AVERAGE_HOURS * 60 * 60);
    ...
    return currentPrice || 0.10; // Default fallback price
  }
  ...
  static async usdToJuno(usdAmount: number): Promise<number> {
    const price = await this.getRollingAveragePrice();
    if (price <= 0) {
  ...
  static getAllFineConfigs(): FineConfig[] {
    return query<FineConfig>(
      'SELECT * FROM fine_config ORDER BY fine_type'
    );
  }
  ...
  static async getPriceInfo(): Promise<{
    ...
    const average = await this.getRollingAveragePrice();
  ...
  static async calculateBailAmount(durationMinutes: number): Promise<number> {
    const perMinuteUsd = this.getFineConfigUsd('jail_per_minute');
    const minimumUsd = this.getFineConfigUsd('jail_minimum');

    const totalUsd = Math.max(minimumUsd, durationMinutes * perMinuteUsd);
    const junoAmount = await this.usdToJuno(totalUsd);
  ...
  static async calculateViolationFine(restriction: string): Promise<number> {
    ...
    const usdAmount = this.getFineConfigUsd(fineType);
    const junoAmount = await this.usdToJuno(usdAmount);
