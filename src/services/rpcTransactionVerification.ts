...
export class RPCTransactionVerification {
  private static readonly RPC_ENDPOINT = config.junoRpcUrl || 'https://rpc.juno.basementnodes.ca';
  ...
  static async fetchTransaction(txHash: string): Promise<{
    ...
    try {
      ...
      const url = `${this.RPC_ENDPOINT}/tx?hash=${formattedHash}&prove=false`;
      ...
      const parsed = await this.parseRPCTransaction(rpcResponse.result);
   ...
   */
  private static async parseRPCTransaction(result: RPCTransactionResponse['result']): Promise<ParsedTransaction> {
    const parsed: ParsedTransaction = {
      hash: result.hash,
      status: result.tx_result.code,
      height: parseInt(result.height),
      transfers: [],
      gasUsed: parseInt(result.tx_result.gas_used),
      gasWanted: parseInt(result.tx_result.gas_wanted)
    };
    ...
    try {
      ...
      if (amountIndex >= 0) {
        ...
        for (let i = searchStart; i < txBytes.length - 2 && i < searchStart + 20; i++) {
          ...
          if (txBytes[i] === 0x12) {
            ...
            if (length > 0 && length <= 20 && i + 2 + length <= txBytes.length) {
              ...
              if (/^[\x20-\x7E]+$/.test(potentialMemo)) {
                ...
                if (!potentialMemo.startsWith('juno1')) {
                  ...
                  if (/^\d+$/.test(potentialMemo) || /^[a-zA-Z0-9_-]+$/.test(potentialMemo)) {
      ...
      if (!parsed.memo) {
        ...
        for (let i = 0; i < txBytes.length - 2; i++) {
          if (txBytes[i] === 0x12) {
            ...
            if (length >= 1 && length <= 15 && i + 2 + length <= txBytes.length) {
        ...
        if (memoFields.length > 0) {
          ...
          const filtered = memoFields.filter(m =>
            m !== '99334' && // gas used
            m !== '122282' && // gas wanted
            m !== '9172' && // fee
    ...
    for (const event of result.tx_result.events) {
      if (event.type === 'transfer') {
        const sender = this.getEventAttribute(event, 'sender');
        const recipient = this.getEventAttribute(event, 'recipient');
        const amountStr = this.getEventAttribute(event, 'amount');

        if (sender && recipient && amountStr) {
          // Parse amount (e.g., "1000000ujuno")
          const amountMatch = amountStr.match(/(\d+)ujuno/);
          if (amountMatch) {
            const ujunoAmount = parseInt(amountMatch[1]);
      ...
      if (event.type === 'tx') {
        const feeStr = this.getEventAttribute(event, 'fee');
        const feePayer = this.getEventAttribute(event, 'fee_payer');

        if (feeStr && feePayer) {
          const feeMatch = feeStr.match(/(\d+)ujuno/);
          if (feeMatch) {
            const ujunoFee = parseInt(feeMatch[1]);
            parsed.fee = {
  ...
  static async verifyDeposit(
    ...
    const result = await this.fetchTransaction(txHash);
  ...
  static async verifyWithdrawal(
    ...
    for (const transfer of tx.transfers) {
      if (transfer.sender === expectedSender && transfer.recipient === expectedRecipient) {
        found = true;
  ...
  static extractMemo(base64Tx: string): string | undefined {
    try {
      ...
      const numericMemo = txString.match(/(?:ujuno[^\d]*)(\d{1,20})(?:[^\d]|$)/);
      if (numericMemo && numericMemo[1]) {
